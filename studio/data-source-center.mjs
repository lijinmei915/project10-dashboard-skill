...
const ui = {
  ...
  name: $("#aiDataSourceName"), meta: $("#aiDataSourceMeta"), sheet: $("#aiDataSheet"), portable: $("#aiDataPortable"),
...
function applySource(next, status = "数据已就绪，可描述目标后生成", meta) {
  ...
  ui.schemaButton.hidden = false;
  ui.refreshButton.hidden = false;
  const remote = ["rest", "postgres"].includes(next.kind);
  ui.refreshButton.textContent = remote ? "立即刷新" : "重新上传";
  ui.scheduleButton.hidden = !remote;
  ui.name.textContent = next.name;
  ui.meta.textContent = meta || `${next.rowCount} 行 · ${next.columnCount} 列 · ${next.quality.issueCount ? `${next.quality.issueCount} 项质量提示` : "质量检查通过"}${next.portable ? " · 随成品携带" : " · 仅服务端使用"}`;
  ...
  ui.generationStatus.textContent = status;
}
...
function clearSource(message) {
  ...
  ui.meta.textContent = message; ui.generationStatus.textContent = message;
}
...
async function payloadFromFile(file) { const lower = file.name.toLowerCase(); const format = lower.endsWith(".xlsx") ? "xlsx" : lower.endsWith(".json") ? "json" : "csv"; return format === "xlsx" ? { name: file.name.replace(/\.[^.]+$/, ""), format, contentBase64: bufferToBase64(await file.arrayBuffer()) } : { name: file.name.replace(/\.[^.]+$/, ""), format, content: await file.text() }; }
...
ui.input.addEventListener("change", async () => { const file = ui.input.files?.[0]; if (!file) return; ui.importButton.disabled = true; ui.refreshButton.disabled = true; ui.name.textContent = file.name; ui.meta.textContent = "正在识别字段和检查数据质量..."; try { const payload = await payloadFromFile(file); if (uploadMode === "refresh" && source()) await refreshData(payload); else await importData(payload); selectedFilePayload = payload; } catch (error) { if (uploadMode === "import") clearSource(error.message || "数据导入失败"); else if (source()) showRefreshFailure(error); } finally { ui.importButton.disabled = false; ui.refreshButton.disabled = false; uploadMode = "import"; ui.input.value = ""; } });
