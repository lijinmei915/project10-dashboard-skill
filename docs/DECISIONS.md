...
- 边界：公开档案只含名称、模型和 HTTPS 接口，真实密钥进入独立凭证仓储；浏览器响应、Workspace、Project、health、日志和便携 Skill 不得获得密钥或 endpoint。当前 file adapter 已支持组织级新增/编辑/删除、持久切换、模型发现和连接测试，并以独立 `0600` 文件保存凭证；这仍不是加密 Secret Manager，也不满足多实例、轮换、KMS 或企业密钥托管，生产适配器完成前不得据此宣称企业凭证平台就绪
